...
// 추상클래스는 객체화하여 사용할 수 없다. 반드시 상속받아 오버라이딩 하여 사용해야 한다.
abstract class 표준Tv { // 추상클래스
    abstract void 켜기 (); //추상메소드
    abstract void 끄기 ();
}

class  샤오미Tv extends 표준Tv{ // 부모클래스에게 상속받은 켜기 메소드가 내용이 없는 추상메소드이기 때문에
                                // 오버라이딩 하지 않으면 실행 할 수 없음
    void 켜기 () {
